        ...
        <div className="flex gap-2">
          ...
          {(booking.status === "inprocess" || booking.status === "pending") && onCancel && (
            <Button
              variant="outline"
              size="sm"
              className="text-xs rounded-lg h-8 text-destructive border-destructive/30 hover:bg-destructive/5 gap-1"
          ...
          {booking.status === "completed" && !booking.review?.isReviewed && onReview && (
            <Button
              size="sm"
              className="text-xs rounded-lg h-8 bg-[#C9AA8B] hover:bg-[#B8956F] text-white gap-1"
...
export default function BookingsPage() {
  ...
  // Review dialog state
  const [bookingToReview, setBookingToReview] = useState<BookingModel | null>(null);
  const [reviewRating, setReviewRating] = useState(5);
  const [reviewComment, setReviewComment] = useState("");
  const [isSubmittingReview, setIsSubmittingReview] = useState(false);

  // ── Redirect unauthenticated users MUST be in useEffect (never in render) ──
  useEffect(() => {
    if (!authLoading && !isLoggedIn) router.push("/login");
  }, [authLoading, isLoggedIn, router]);

  // ── Wait for auth to resolve before rendering (prevents premature null return
  //    that would wipe all dialog state like bookingToReview) ──
  if (authLoading) {
    return (
  ...
  const submitReview = () => {
    ...
    reviewMutation.mutate(
      {
        salonId: bookingToReview.salon?.salonId || "",
      ...
      },
      {
        ...
        onSettled: () => setIsSubmittingReview(false),
      }
    );
  };
          ...
          {isLoading ? (
            ...
            <Tabs defaultValue="inprocess">
              ...
              <TabsContent value="inprocess" className="mt-4 space-y-3">
                {inprocess.length > 0 ? (
                  inprocess.map((b) => (
                    <BookingCard key={b.bookingId} booking={b} t={t} onCancel={handleCancelClick} onReview={handleReviewClick} />
                  ))
                ) : (
              ...
              <TabsContent value="completed" className="mt-4 space-y-3">
                {completed.length > 0 ? (
                  completed.map((b) => (
                    <BookingCard key={b.bookingId} booking={b} t={t} onReview={handleReviewClick} />
                  ))
                ) : (
                  <div className="text-center py-16">
                    <p className="text-muted-foreground">{t.userBookings.noBookings}</p>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="py-2">
            <p className="text-sm text-muted-foreground mb-4">
              {t.userBookings.cancelSubtitle} at{" "}
              <span className="font-semibold text-foreground">
                {bookingToCancel?.salon?.shopName || "the salon"}
              </span>.
            </p>
            <div className="space-y-3 mb-4">
              {predefinedReasons.map((reason) => (
                ...
                >
                  <div
                    className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${selectedReason === reason
                      ? "border-[#C9AA8B]"
                      : "border-border group-hover:border-[#C9AA8B]/50"
                      }`}
                  >
                    {selectedReason === reason && (
                      <div className="w-2.5 h-2.5 rounded-full bg-[#C9AA8B]" />
                    )}
                  </div>
                  <span
                    className={`text-sm ${selectedReason === reason ? "text-foreground font-medium" : "text-muted-foreground"
                      }`}
                  >
                    {reason}
                  </span>
      ...
      <Dialog open={!!bookingToReview} onOpenChange={(open) => !open && setBookingToReview(null)}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="py-2 space-y-4">
            <div>
              <p className="text-sm text-muted-foreground mb-3">{t.review.ratingLabel}</p>
              <div className="flex gap-2">
                {[1, 2, 3, 4, 5].map((star) => (
                  <button
                    key={star}
            ...
            <div>
              <p className="text-sm text-muted-foreground mb-2">{t.review.commentLabel}</p>
              <Textarea
          ...
          <DialogFooter className="mt-2">
            <Button variant="outline" onClick={() => setBookingToReview(null)} disabled={isSubmittingReview}>
              Cancel
            </Button>
            <Button
